	...
	bool Wrapper::Window::IsVSyncEnable() const
{
	...
	std::string Wrapper::Window::GetNonPrintableKeyName(int key)
	{
		switch (key) {
	...
	std::string Wrapper::Window::GetKeyName(int key)
	{
		auto value = glfwGetKeyName(key, 0);
		if (value == nullptr)
			return GetNonPrintableKeyName(key);
		return value;
	}
