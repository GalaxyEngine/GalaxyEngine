...
#pragma endregion
    ...
    void Resource::Shader::Load()
    {
        ...
        if (parser.IsFileOpen())
        {
            ...
            {
                Weak<GeometryShader> geometryShader = ResourceManager::GetOrLoad<
                    Resource::GeometryShader>(geometryUUID);
                SetGeometry(geometryShader.lock(), thisShader);
            }
            Core::UUID fragmentUUID = parser["Fragment"].As<uint64_t>();
            if (fragmentUUID != UUID_NULL)
            {
                Weak<FragmentShader> fragmentShader = ResourceManager::GetOrLoad<
                    Resource::FragmentShader>(fragmentUUID);
    ...
    void Resource::Shader::Send()
    {
        ...
        if (p_hasBeenSent.load() && !p_isAVariant)
        {
            const auto weak_this = Resource::ResourceManager::GetResource<
                Resource::Shader>(GetFileInfo().GetFullPath());
    ...
    {
        Shared<BaseShader> shader = GetVertex().lock();
        if (shader)
            shader->RemoveShader(this);

        p_subShaders.vertexShader = vertexShader;
        if (vertexShader)
    ...
    {
        Shared<BaseShader> shader = GetGeometry().lock();
        if (shader)
            shader->RemoveShader(this);

        p_subShaders.geometryShader = geometryShader;
        if (geometryShader)
    ...
    {
        Shared<BaseShader> shader = GetFragment().lock();
        if (shader)
        {
            shader->RemoveShader(this);
        }
        p_subShaders.fragmentShader = fragmentShader;
        if (fragmentShader)
    ...
    void Resource::Shader::Recompile() const
    {
        ...
        if (const Shared<VertexShader> vertShader = GetVertex().lock())
        {
            vertShader->Recompile();
        }
        if (const Shared<GeometryShader> geomShader = GetGeometry().lock())
        {
            geomShader->Recompile();
        }
        if (const Shared<FragmentShader> fragShader = GetFragment().lock())
        {
    ...
    Shared<Resource::Shader> Resource::Shader::Create(const Path& vertPath, const Path& fragPath)
    {
        ...
        std::string shaderPath = vertexShader.lock()->GetFileInfo().GetFullPath().string() + " + " + fragShader.lock()->
            GetFileInfo().GetRelativePath().string();
    ...
    void Resource::Shader::Serialize(CppSer::Serializer& serializer) const
    {
        ...
        const Shared<VertexShader> vertex = GetVertex().lock();
        const Shared<GeometryShader> geometry = GetGeometry().lock();
        const Shared<FragmentShader> fragment = GetFragment().lock();
    ...
    {
        auto new_end = std::ranges::remove_if(p_shaders,
