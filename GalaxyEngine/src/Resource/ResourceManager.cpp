...
namespace GALAXY
{
    ...
    void Resource::ResourceManager::UpdateFileWatch()
    {
        while (!Core::ThreadManager::ShouldTerminate())
        {
            ...
            const Path assetPath = GetAssetPath();
    ...
    inline Weak<Resource::IResource> Resource::ResourceManager::ReloadResource(
    ...
    Weak<Resource::IResource> Resource::ResourceManager::GetResource(const Core::UUID& uuid)
    {
        if (uuid == UUID_NULL)
            return {};
        auto resource = std::ranges::find_if(m_instance->m_resources,
                                             [&](const std::pair<Path, Shared<IResource>>& _resource)
                                             {
    ...
    void Resource::ResourceManager::ReadCache()
    {
        ...
        * if yes continue
        ...
        for (auto& resource : m_resources)
        {
            if (resource.second->GetFileInfo().GetResourceDir() == ResourceDir::Editor || !std::filesystem::exists(
                resource.second->GetFileInfo().GetFullPath()))
            {
    ...
    void Resource::ResourceManager::CreateCache()
    {
        ...
        for (auto& val : m_resources | std::views::values)
        {
            if (val->GetFileInfo().GetResourceDir() == ResourceDir::Editor || !std::filesystem::exists(
                val->GetFileInfo().GetFullPath()))
                continue;
