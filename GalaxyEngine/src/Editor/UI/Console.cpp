...
namespace GALAXY {
	...
	void Editor::UI::Console::Draw()
	{
		...
		if (ImGui::Begin("Console", &p_open, ImGuiWindowFlags_NoTitleBar))
		{
			...
			ImGui::SameLine();
			ImGui::PushID(0);
			Wrapper::UI::TextureToggleButtonWithText(IconManager::InfoIcon, std::to_string(m_infoNumber).c_str(), &m_infoCheckbox, size);
			ImGui::PopID();
			ImGui::SameLine();
			ImGui::PushID(1);
			Wrapper::UI::TextureToggleButtonWithText(IconManager::WarningIcon, std::to_string(m_warningNumber).c_str(), &m_warningCheckbox, size);
			ImGui::PopID();
			ImGui::SameLine();
			ImGui::PushID(2);
			Wrapper::UI::TextureToggleButtonWithText(IconManager::ErrorIcon, std::to_string(m_errorNumber).c_str(), &m_errorCheckbox, size);
			ImGui::PopID();
			...
			Wrapper::UI::Splitter(false, 2, &topSize, &bottomSize, 10, 10);
			...
			if (m_textSelected != INDEX_NONE && m_textSelected < m_texts.size())
			{
				...
				Wrapper::UI::TextSelectable(prefix + message, color);
				Wrapper::UI::TextSelectable(fileLine);
			}
			ImGui::EndChild();
		}
		ImGui::End();
