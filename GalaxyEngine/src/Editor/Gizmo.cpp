...
namespace GALAXY
{
   ...
   void Editor::Gizmo::Update()
   {
      ...
      if (Input::IsMouseButtonPressed(MouseButton::BUTTON_1) && EditorUI::EditorUIManager::GetInstance()->GetSceneWindow()->IsHovered())
      {
   ...
   void Editor::Gizmo::SetGameObject(Weak<Core::GameObject> object)
   {
    ...
    float ClosestDistanceBetweenLines(Physic::Ray l1, Physic::Ray& l2)
    {
        ...
        return (p1 - p2).Length(); // distance between closest points
    }

   float Editor::Gizmo::CalculateRayDistance(const Physic::Ray& mouseRay, int index)
   {
      Vec3f axisVector = Vec3f::Zero();
      axisVector[index] = 1;

      m_translateRays[index].origin = m_object.lock()->Transform()->GetWorldPosition();
      m_translateRays[index].direction = m_object.lock()->Transform()->GetWorldRotation() * axisVector;

      return ClosestDistanceBetweenLines(mouseRay, m_translateRays[index]);
   }

   void Editor::Gizmo::HandleAxis(const Physic::Ray &mouseRay)
   {
