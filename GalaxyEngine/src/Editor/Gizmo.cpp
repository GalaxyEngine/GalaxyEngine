...
static bool RayCircleIntersection(const Physic::Ray& ray, Circle circle, Vec3f& intersectionPoint)
{
...
static Vec3f RayPlaneIntersection(const Physic::Ray& ray, Physic::Plane plane, const Vec3f& point)
{
    ...
    float distance = plane.normal.Dot(point - ray.origin) / dotProduct;
    if (distance < 0.f)
    {
...
static float ClosestDistanceLineCircle(const Physic::Ray& line, const Circle& c, Vec3f& point, bool enablePlane = false)
{
    ...
    {
        point = RayPlaneIntersection(line, f, c.center);
            ...
            case GizmoType::Translation:
                {
                    ...
                    m_currentPosition = m_translateRays[i].origin + m_translateRays[i].direction * m_translateRays[i].
                        scale;
            ...
            case GizmoType::Rotation:
                {
                    ...
                    float angle = -AngleBetweenVectorsInAxis(m_currentPosition - m_rotateCircle[i].center,
                                                             m_startPosition - m_rotateCircle[i].center,
                                                             m_mode == Space::Local
                                                                 ? m_gizmoRotation * axisVector
                                                                 : axisVector);
        ...
        case GizmoType::Scale:
            {
                ...
                for (size_t i = startIndex; i < endIndex; i++)
                {
                    Vec4f color(0, 0, 0, 1);
                    color[i] = 1;
                    Vec3f endLocation = m_translateRays[i].origin + m_translateRays[i].direction * (m_gizmoLength -
                        cubeScale.x);
        ...
        case GizmoType::Translation:
            {
                ...
                for (size_t i = startIndex; i < endIndex; i++)
                {
                    Vec4f color(0, 0, 0, 1);
                    color[i] = 1;
                    m_renderer->DrawLine(m_translateRays[i].origin,
        ...
        case GizmoType::Rotation:
            {
                ...
                for (size_t i = startIndex; i < endIndex; i++)
                {
                    Vec4f color(0, 0, 0, 1);
                    color[i] = 1;
                    m_renderer->DrawWireCircle(m_rotateCircle[i].center, m_rotateCircle[i].orientation, m_gizmoLength,
                                               32, color, 3.f);
                }
                if (m_gizmoClicked)
                {
